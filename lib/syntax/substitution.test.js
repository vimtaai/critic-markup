...
describe("substitution", () => {
  ...
  it("should escape HTML in rendered content", () => {
    const input = "Lorem {~~<b>old</b>~><i>new</i>~~} ipsum";
    const expectedOutput =
      "Lorem <del>&lt;b&gt;old&lt;/b&gt;</del><ins>&lt;i&gt;new&lt;/i&gt;</ins> ipsum";
  ...
  it("should pass through HTML unescaped when escape option is false", () => {
    const input = "Lorem {~~<b>old</b>~><i>new</i>~~} ipsum";
    const expectedOutput = "Lorem <del><b>old</b></del><ins><i>new</i></ins> ipsum";

    const renderedMatches = renderMatches(input, substitution, { escape: false });
    assert.strictEqual(renderedMatches, expectedOutput);
  });
});
