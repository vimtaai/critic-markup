...
describe("comment", () => {
  ...
  it("should escape HTML in rendered content", () => {
    const input = `Lorem ipsum dolor sit {>><script>alert(1)</script><<}.`;
    const expectedOutput = `Lorem ipsum dolor sit <span class="critic comment">&lt;script&gt;alert(1)&lt;/script&gt;</span>.`;
  ...
  it("should pass through HTML unescaped when escape option is false", () => {
    const input = `Lorem ipsum dolor sit {>><b>comment</b><<}.`;
    const expectedOutput = `Lorem ipsum dolor sit <span class="critic comment"><b>comment</b></span>.`;

    const renderedMatches = renderMatches(input, comment, { escape: false });
    assert.strictEqual(renderedMatches, expectedOutput);
  });
});
