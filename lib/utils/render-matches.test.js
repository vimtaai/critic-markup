...
describe("parseMatches", () => {
  ...
  it("should pass options through to render", () => {
    const tokenType = {
      regex: new RegExp(/(?<foo>ab)/, "gs"),
      validate: () => true,
      render: (token, options) =>
        options?.shout ? token.content.foo.toUpperCase() : token.content.foo,
    };
    const inputText = "A ab B ab";

    const renderedMatches = renderMatches(inputText, tokenType, { shout: true });
    assert.strictEqual(renderedMatches, "A AB B AB");
  });
});
